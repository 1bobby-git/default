...
result_dir = root / "apti-results-v5"
...
shutil.rmtree(root / "apti-handshake-v5", ignore_errors=True)
subprocess.run(["git", "add", "apti-results-v5", "apti-handshake-v5"], cwd=root, check=False)
subprocess.run(["git", "commit", "-m", f"Store encrypted Apti v5 result {run_id}"], cwd=root, check=False)
subprocess.run(["git", "push", "origin", "HEAD:analysis/apti-emulator-20260901"], cwd=root, check=True)

for secret_path in (credentials_file, key_file, Path("/dev/shm/apti-v5/private.pem")):
    try:
